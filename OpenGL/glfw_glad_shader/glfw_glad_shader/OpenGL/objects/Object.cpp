...
bool Object::AddData2VaoUnit(char* DataPoints, char* DataColors)
{
	if (!VaoUnitVectorIdle.empty())
	{
		...
		//加载数据
		BindData2VaoUnit(DataPoints, DataColors);
...
void Object::BindData2VaoUnit(char* DataPoints, char* DataColors)
{
	/*
	...
	*/
	...
	glBindBuffer(GL_ARRAY_BUFFER, p_VaoUnit->GetVbo()[0]);
	glBufferData(GL_ARRAY_BUFFER, 128 * 128 * 3 * sizeof(float), DataPoints, GL_DYNAMIC_DRAW);
	...
	glBindBuffer(GL_ARRAY_BUFFER, p_VaoUnit->GetVbo()[1]);
	glBufferData(GL_ARRAY_BUFFER, 128 * 128 * 4 * sizeof(float), DataColors, GL_DYNAMIC_DRAW);
...
void Object::RenderVaoUnit()
{
	...
	for (auto it = VaoUnitVectorShow.cbegin(); it != VaoUnitVectorShow.cend(); ++it)
	{
		...
		*/
		...
		// 12*3 indices starting at 0 -> 12 triangles
		glDrawArrays(GL_POINTS, 0, 128 * 128 * 3);
