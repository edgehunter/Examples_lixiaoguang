...
void VaoEngine::Init(Shader* p_Shader)
{
	...
	float VerticesTriangleEngine[] = {
		0.5f, -0.5f, -5.0f, 1.0f,  // bottom right
		-0.5f, -0.5f, -5.0f, 1.0f,  // bottom left
		0.0f, 0.5f, -5.0f, 1.0f   // top 
	};
	...
	pointNum = 128 * 128;
	VerticesPoints = new float[pointNum*4];
	...
	{
		VerticesPoints[i * 4] = (float)(rand() % 80);
		VerticesPoints[i * 4 + 1] = (float)(rand() % 80);
		VerticesPoints[i * 4 + 2] = (float)(rand() % 80);
		VerticesPoints[i * 4 + 3] = 1.0f;//(float)(rand() % 80);
