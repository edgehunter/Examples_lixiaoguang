...
void Object::openglInitialization() {
  ...
  GLfloat *vertexArray = new GLfloat[this->vertices.size()*this->elms];
   ...
   */
  Triangle* t;
  vector<unsigned int> v, vn;
  map<unsigned int,unsigned int> vertexMap; // mapping of vertex index to element index
  unsigned int q = 0; // element counter for current vertex
  for (unsigned int i = 0; i < this->triangles.size(); i++) {
    ...
    for (unsigned int k = 0; k < 3; k++) {
      unsigned int a = q*this->elms;
      unsigned int vi = v[k]-1;
      unsigned int vni = vn[k]-1;
      if (vertexMap.find(vi) == vertexMap.end()) {
        // add the vertex to the set and increment the counter
        // the counter (q) is the element index
        q++;
        vertexMap.insert(pair<unsigned int,unsigned int>(vi,q));
   ...
   */
  unsigned int *elementArray = new  unsigned int[this->triangles.size() * 3];
  for (unsigned int i = 0; i < this->triangles.size(); i++) {
    unsigned int a = i*3;
    vector<unsigned int> v = this->triangles[i].getVertices();
    for (unsigned int j = 0; j < 3; j++) {
...
void Object::outputToObj(string filename) const {
  ...
  // vertex normals
  for (unsigned int i = 0; i < this->vertexNormals.size(); i++) {
  ...
  for (unsigned int i = 0; i < this->triangles.size(); i++) {
    vector<unsigned int> v = this->triangles[i].getVertices();
    vector<unsigned int> vn = this->triangles[i].getVertexNormals();
    file << "f " 
         << v[0] << "//" << vn[0] << " "
         << v[1] << "//" << vn[1] << " " 
         << v[2] << "//" << vn[2] << "\n";
...
glm::vec3 Object::surfaceNormal(Triangle t) const {
  vector<unsigned int> v = t.getVertices();
