...
{    
	...
	pIOCPModel->_ShowMessage(L"工作者线程启动，ID: %d.",nThreadNo);
...
bool CIOCPModel::Start()
{
	...
	{
		this->_ShowMessage(L"Listen Socket初始化完毕.");
	}
...
bool CIOCPModel::_InitializeListenSocket()
{
	...
	{
		this->_ShowMessage(L"初始化Socket失败，错误代码: %d.\n", WSAGetLastError());
		return false;
	}
	else
	{
	...
	{  
		this->_ShowMessage(L"绑定 Listen Socket至完成端口失败！错误代码: %d/n", WSAGetLastError());  
	...
	//ServerAddress.sin_addr.s_addr = inet_addr((char*)m_strIP.GetBuffer(m_strIP.GetLength()));
	ServerAddress.sin_port = htons(m_nPort);                          

	// 绑定地址和端口
	if (SOCKET_ERROR == bind(m_pListenContext->m_Socket, (struct sockaddr *) &ServerAddress, sizeof(ServerAddress))) 
	{
		this->_ShowMessage(L"bind()函数执行错误.\n");
		return false;
	}
	else
	{
	...
	{
		this->_ShowMessage(L"Listen()函数执行出现错误.\n");
		return false;
	}
	else
	{
	...
	if(SOCKET_ERROR == WSAIoctl(
		...
		NULL))  
	{  
		this->_ShowMessage(L"WSAIoctl 未能获取AcceptEx函数指针。错误代码: %d\n", WSAGetLastError()); 
		this->_DeInitialize();
		return false;  
	}  
	...
	if(SOCKET_ERROR == WSAIoctl(
		...
		NULL))  
	{  
		this->_ShowMessage(L"WSAIoctl 未能获取GuidGetAcceptExSockAddrs函数指针。错误代码: %d\n", WSAGetLastError());  
		this->_DeInitialize();
		return false; 
	}  
...
bool CIOCPModel::_PostAccept( PER_IO_CONTEXT* pAcceptIoContext )
{
	...
	{  
		_ShowMessage(L"创建用于Accept的Socket失败！错误代码: %d", WSAGetLastError()); 
		return false;  
	} 
	...
	{  
		...
		{  
			_ShowMessage(L"投递 AcceptEx 请求失败，错误代码: %d", WSAGetLastError());  
			return false;  
		}  
	} 
...
bool CIOCPModel::_PostRecv( PER_IO_CONTEXT* pIoContext )
{
	...
	{
		this->_ShowMessage(L"投递第一个WSARecv失败！");
		return false;
	}
	return true;
}
...
bool CIOCPModel::_AssociateWithIOCP( PER_SOCKET_CONTEXT *pContext )
{
	...
	if (NULL == hTemp)
	{
		this->_ShowMessage((L"执行CreateIoCompletionPort()出现错误.错误代码：%d"),GetLastError());
		return false;
	}
