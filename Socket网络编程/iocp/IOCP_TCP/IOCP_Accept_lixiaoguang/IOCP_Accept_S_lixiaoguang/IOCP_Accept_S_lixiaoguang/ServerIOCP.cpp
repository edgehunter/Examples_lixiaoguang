...
int main()
{
	...
	ServerAddress.sin_addr.S_un.S_addr = inet_addr("192.168.1.11"); //WinSock will supply address
...
void AcceptConnection(SOCKET ListenSocket)
{
	...
	{
		WriteToConsole("\nError occurred while accepting socket: %d,Error Code: %ld.", Socket, WSAGetLastError());
	}

	//Display Client's IP
	WriteToConsole("\nClient connected from: %s, socket: %d", inet_ntoa(ClientAddress.sin_addr), Socket);
	...
	pClientContext->SetAddress(ClientAddress);
	...
	if (true == AssociateWithIOCP(pClientContext))
	{
		...
		{
			WriteToConsole("\nError in Initial Post, socket: %d, Error Code %ld.", Socket, WSAGetLastError());
		}
	}
}
...
bool AssociateWithIOCP(CClientContext   *pClientContext)
{
	...
	if (NULL == hTemp)
	{
		WriteToConsole("\nError occurred while executing CreateIoCompletionPort(), address: %s, socket: %d, Error Code %ld.", inet_ntoa(pClientContext->GetAddress().sin_addr), pClientContext->GetSocket(), WSAGetLastError());
...
DWORD WINAPI WorkerThread(LPVOID lpParam)
{
	...
	while (WAIT_OBJECT_0 != WaitForSingleObject(g_hShutdownEvent, 0))
	{
		...
		if ((FALSE == bReturn) || ((TRUE == bReturn) && (0 == dwBytesTransfered)))
		{
			...
			WriteToConsole("\nClient close: %s, socket: %d", inet_ntoa(pClientContext->GetAddress().sin_addr), pClientContext->GetSocket());
		...
		case OP_READ:
			...
			pClientContext->IncrSentBytes(dwBytesTransfered);//dwBytes，这个是不是放在这里
			...
			if (pClientContext->GetSentBytes() < pClientContext->GetTotalBytes())
			{
				...
				{
					WriteToConsole("\nThread %d: Error occurred while executing WSASend(), address: %s, Socket: %d, Error Code %ld.", nThreadNo, inet_ntoa(pClientContext->GetAddress().sin_addr), pClientContext->GetSocket(), WSAGetLastError());
			...
			else
			{
				...
				if ((SOCKET_ERROR == nBytesRecv) && (WSA_IO_PENDING != WSAGetLastError()))
				{
					WriteToConsole("\nThread %d: Error occurred while executing WSARecv(), address: %s, Socket: %d, Error Code %ld.", nThreadNo, inet_ntoa(pClientContext->GetAddress().sin_addr), pClientContext->GetSocket(), WSAGetLastError());
