// IOCP_UDP_C_lixiaoguang.cpp : Defines the entry point for the console application.
//
...
#define DEFAULT_PORT            "5005"          // Default server port
#define DEFAULT_PROTO           SOCK_DGRAM     // Default protocol (UDP)
...
int __cdecl main(int argc, char **argv)
{
	...
	struct addrinfo *results = NULL,
		*addrptr = NULL,
		hints;
	char            *server_name = "192.168.1.122",
		*port = DEFAULT_PORT,
	...
	Header* m_Header_Send = new Header;
	Header* m_Header_Recv = new Header;
	...
	for (;;)
	{
		//设置序号
		m_Header_Send->x = 0;
		m_Header_Send->y = loopcount % 1565;

		//大小端转换
		changeBigLittle(BigLittle, m_Header_Send);

		//内存复制
		memcpy(Buffer, m_Header_Send, sizeof(Header));

		loopcount++;

		// Send the data
		retval = send(conn_socket, Buffer, DEFAULT_BUFFER_LEN, 0);//lstrlenA(Buffer) + 1, 0);
		if (retval == SOCKET_ERROR)
		{
		...
		//内存复制
		memcpy(m_Header_Recv, Buffer, sizeof(Header));

		//大小端转换
		changeBigLittle(BigLittle, m_Header_Recv);
		
		//printf("read %d bytes, data [%d] from server\n", retval, m_Header_Recv->y);

		//错误判断
		changeBigLittle(BigLittle, m_Header_Send);
		if (m_Header_Recv->y != m_Header_Send->y)
		{
			printf("产生错误\n");
