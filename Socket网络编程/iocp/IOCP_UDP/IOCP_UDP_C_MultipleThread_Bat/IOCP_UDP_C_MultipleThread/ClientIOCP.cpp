...
int main()
{
	...
	int nPortNo = 5005;
	char* Connect_ServerAddress = "192.168.1.11";
	...
	{
		bConnectedSocketCreated = CreateConnectedSocket(&(pThreadInfo[ii].m_Socket), Connect_ServerAddress, nPortNo);
...
bool CreateConnectedSocket(SOCKET *pSocket, char *szHost, int nPortNo)
{
	...
	*pSocket = socket(AF_INET, SOCK_DGRAM, IPPROTO_UDP);
	...
	ServerAddress.sin_addr.S_un.S_addr = inet_addr(szHost);
...
DWORD WINAPI WorkerThread(LPVOID lpParam)
{
	...
	char szTemp_Send[MAX_BUFFER_LEN];
	char szTemp_Recv[MAX_BUFFER_LEN];
	...
	{
		sprintf(szTemp_Send, "%d. %s", ii + 1, pThreadInfo->m_szBuffer);

		//Send the message to the server, include the NULL as well
		nBytesSent = send(pThreadInfo->m_Socket, szTemp_Send, strlen(szTemp_Send), 0);
		...
		nBytesRecv = recv(pThreadInfo->m_Socket, szTemp_Recv, MAX_BUFFER_LEN, 0);
		...
		//×Ö·û´®±È½Ï
		if (strncmp(szTemp_Send, szTemp_Recv, 10))
		{
			//Display the server message
			WriteToConsole("\n strcmp(szTemp_Send = %s, szTemp_Recv = %s) ERROR!", szTemp_Send, szTemp_Recv);
		}
		//else
		//{
		//	WriteToConsole("\n strcmp(szTemp_Send = %s, szTemp_Recv = %s) Right!", szTemp_Send, szTemp_Recv);
		//}
	...
	//shutdown(pThreadInfo->m_Socket, SD_SEND);
	...
	*/
	...
	//closesocket(pThreadInfo->m_Socket);
